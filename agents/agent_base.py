...
from agents.agent_interface import AgentBase
...
class Agent(AgentBase):
    def __init__(
        ...
        containerization_platform: str,
        jinja2_command_builder: Jinja2CommandBuilder = None,
    ):
    ...
    def run_socat_dump_comms(
        ...
        for conn in network_connections:
            if conn.state == "ESTABLISHED":
                command = self.jinja2_command_builder.generate_socat_dump_command(
    ...
    def run_socat_sinkhole_comms(
        ...
        for conn in network_connections:
            if conn.state == "ESTABLISHED":
                command = self.jinja2_command_builder.generate_socat_sinkhole_command(
