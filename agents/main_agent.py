...
from models.
from agent_utils import parse_procinfo_output, get_network_connections, get_process_info
from .jinja2_templates import generate_socat_dump_command, generate_socat_sinkhole_command
...
class InterrogationAgent:
    ...
    def run_socat_dump_comms(self, network_connections: List[NetworkConnection]) -> None:
        for conn in network_connections:
            if conn.state == "ESTABLISHED":
                command = generate_socat_dump_command(conn.address, conn.port, "dump.pcap")
    ...
    def run_socat_sinkhole_comms(self, network_connections: List[NetworkConnection], sinkhole_address: str, sinkhole_port: int) -> None:
        for conn in network_connections:
            if conn.state == "ESTABLISHED":
                command = generate_socrat_sinkhole_command(conn.address, conn.port, sinkhole_address, sinkhole_port)
                subprocess.run(command.split(), check=True)
    ...
    @classmethod
    def from_pid(cls, pid: int, virtual_env_path: str, containerization_platform: str) -> "Agent":
        process_info = get_process_info(pid)
        network_connections = get_network_connections(pid)
        return cls(virtual_env_path, containerization_platform, process_info, network_connections)
    ...
    def generate_action(process_info: ProcessInfo) -> Action:
        if process_info.threat_level == ThreatLevel.NONE or process_info.pid is None:
            return Action.DISABLE_SERVICE  # Default action when no threat is detected or PID is unknown

        elif process_info.command in ["launchctl", "bootout"]:
            return Action.BOOTOUT  # Stop the launch daemon or bootout process directly

        elif any(conn.state == "ESTABLISHED" for conn in process_info.network_connections):
            if process_info.pid:
                return Action.KILL_PROCESS | Action.SOCAT_DUMP
            else:
                return Action.SOCAT_DUMP  # Dump network traffic without killing the process

        elif any(conn.command == "soc" for conn in process_info.network_connections):
            return Action.SOCAT_SINKHOLE  # Sinkhole socat connections to stop all communications

        else:
            if process_info.pid is not None:
                return Action.KILL_PROCESS | Action.LAUNCHCTL_REMOVE
            else:
                return Action.LAUNCHCTL_REMOVE  # Remove the offending launch daemon (if applicable) without killing any processes
